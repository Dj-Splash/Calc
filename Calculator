...
class Calculator {
    ...
    static String calc(String inp) throws com.stanislav.InputException {
        inp = inp.replaceAll(" ", ""); // Удаляем все пробелы

        /* Проверка на пустой ввод */
        if(inp.isEmpty()){
            throw new com.stanislav.InputException("Ошибка ввода! Пустая входная строка.");
        }
        ...
        if(inputs.isEmpty()){
            throw new com.stanislav.InputException("throws Exception //т.к. строка не является математической операцией.");
        }else if(inputs.size() == 1){
            throw new com.stanislav.InputException("Ошибка ввода! Отсутствуют значения.");
        }else if(inputs.size() == 2){
            throw new com.stanislav.InputException("Ошибка ввода! Одно из значений отсутствует.");
        }
        ...
        try{
            ...
            try{
                ...
                throw new com.stanislav.InputException("Ошибка ввода! Неожиданное значение '" + inputs.get(2) + "'.");
            }
        }
        catch (NumberFormatException e){
            try{
                // Пробуем получить римское число
                left = Number.toRoman(inputs.get(0)).getNumber();
            }
            catch (IllegalArgumentException e1){
                throw new com.stanislav.InputException("Ошибка ввода! Неожиданное значение '" + inputs.get(0) + "'.");
            }
            try{
                // Пробуем получить второе римское число
                right = Number.toRoman(inputs.get(2)).getNumber();
            }
            catch (IllegalArgumentException e1){
                throw new com.stanislav.InputException("Ошибка ввода! Неожиданное значение '" + inputs.get(2) + "'.");
            }
        ...
        if (!isValidNumber(left) || !isValidNumber(right)) {
            ...
            throw new com.stanislav.InputException("Ошибка ввода! Недопустимое значение.");
            ...
            switch (inputs.get(1).charAt(0)) {
                ...
                case '/':
                    ...
                    break;
                default:
                    throw new com.stanislav.InputException("Ошибка ввода! Неожиданный оператор '" + inputs.get(1) + "'.");
            }
            /* Проверяем результат на принадлежность диапазону [1..10] */
            if(isValidNumber(result)){
                return isRoman ? Number.toRoman(result).toString() :
